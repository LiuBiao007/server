...
local field        = require "coredb.com.field"
local where_class  = require "coredb.com.where"
local type         = type
...
handler.getOptions = _getOptions
...
handler.setOptions = _setOptions
...
handler.removeOptions = _removeOptions

-- new语法构造新对象
function handler.new(self)
...
function handler.table(self, table)
    ...
    if table:find("[A-Z]") then

        utils.exception(string.format("[table] name [%s] macth upper name.", table))
    end    
...
function handler.alias(self, alias)
...
function handler.field(self, fields)
...
function handler.fieldRaw(self, fields)
    -- field模块设置处理字段后返回已处理好的字段数组列表
...
function handler.join(self, table, condition, operate, binds)
    -- 检查必选参数
...
function handler.where(self, column, operate, condition)
    -- 传递给内部where对象处理
...
-- @param string operate 操作符
-- @param string|array condition 操作条件
function handler.whereOr(self, column, operate, condition)
    -- 传递给内部where对象处理
...
-- IS NULL 快捷用法
-- @param string column 字段名称
function handler.whereNull(self, column)
...
-- IS NOT NULL 快捷用法
-- @param string column 字段名称
function handler.whereNotNull(self, column)
...
-- @param string column 字段名称
-- @param string|array condition 操作条件
function handler.whereIn(self, column, condition)
...
-- @param string column 字段名称
-- @param string|array condition 操作条件
function handler.whereNotIn(self, column, condition)
...
-- @param string column 字段名称
-- @param string|array condition 操作条件
function handler.whereBetween(self, column, condition)
...
-- @param string column 字段名称
-- @param string|array condition 操作条件
function handler.whereNotBetween(self, column, condition)
...
-- @param string column 字段名称
-- @param string|array condition 操作条件
function handler.whereLike(self, column, condition)
...
-- @param string column 字段名称
-- @param string|array condition 操作条件
function handler.whereNotLike(self, column, condition)
...
-- @param string column 字段名称
-- @param string|array condition 操作条件
function handler.whereExp(self, column, condition)
...
function handler.order(self, column, sorted)
    -- upper case
...
function handler.data(self, column, value)
    -- 数组形式设置data
    if "table" == type(column) then
...
function handler.limit(self, offset, length)
...
function handler.page(self, page, page_size)
    page      = tonumber(page)
...
function handler.group(self, column)
    -- check
...
function handler.having(self, condition)
    -- 仅支持字符串参数
...
function handler.distinct(self, distinct)
...
-- 设置锁机制
-- @param bool|string lockhandlerode 传入true则是FOR UPDATE锁，传入字符串则是特殊的锁，譬如：lock in share mode
function handler.lock(self, lockhandlerode)
    if true == lockhandlerode then
        self.options.lock = 'FOR UPDATE'
    end
    if 'string' == type(lockhandlerode) then
        self.options.lock = utils.trim(lockhandlerode)
    end
    return self
end
