...
function ws:recv()
    ...
    while true do
        ...
        if not success then
            mylog.info("=====>>> on recv success:%s message:%s", success, message)
            return success, message
        end
        if type(message) ~= 'string' then 
            mylog.info(">>>>>>> message type %s success %s", type(message), success)
            mylog.info("======>>> on recv message:%s final:%s", tool.dump(message), tool.dump(final))
            return false 
        end
        if final then
            data = data .. message
...
function ws:recv_frame()
    ...
    if hasClose then 
        mylog.info("a0------------->has close")
        --return false, nil, 'has close' 
    end
    local data, err = read(self.id, 2)
    
    if not data then
        mylog.info("a1------------->Read first 2 byte error")
    ...
    if reserved_bits then
        ...
        mylog.info("a------------->Reserved_bits show using undefined extensions")
    ...
    elseif payloadlen == 126 then
        ...
        if not h_data then
            mylog.info("d------------->Payloadlen 126 read true length error")
        ...
        if not l_data then
            mylog.info("e------------->Payloadlen 127 read true length error")
    ...
    if mask_frame then
        ...
        if not mask then
            mylog.info("f------------->Masking Key read error")
    ...
    if frame_length > 0 then
        ...
        if not fdata then
            mylog.info("f------------->Payload data read error")
    ...
    if not final_frame then
        ...
        mylog.info(">>>>>>>>>>>>>>>>>>>>>>>>>> frame_opcode = %s", frame_opcode)
...
function ws:start()
    ...
    while true do
        --if hasClose then break end
        local ok, message, err = pcall(self.recv, self)
        if not ok or not message then
            mylog.info('[ok: %s]recv eror: %s %s id : %s', ok, message, err, self.id)
            pcall(self.handler.on_close, self, 118, '[websocket] close ' .. self.id)
            break
        end
    end
end
