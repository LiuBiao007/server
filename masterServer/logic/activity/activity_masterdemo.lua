...
local masterActivity = require "afw.masterActivity"
local demo = class("masterDemo1", masterActivity)
...
------------------------协议操作-----------------------
function demo:mkick(playerId, a, b, c)

	print("global ", a, b, c)
	local activityState = self:checkAndNewState(playerId)
	activityState.data.test3.c.m = a
